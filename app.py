...
with st.sidebar:
    ...
    st.divider()
    st.header("⚡ 快速补剂")
    opt_bcaa = st.checkbox("🥤 练中 BCAA")
    opt_protein = st.checkbox("🥛 练后 蛋白粉")
...
if st.button("🚀 生成详细报告", type="primary"):
    ...
    try:
        ...
        **不要乘序号！** 单组容量 = 重量 * 次数。
        ...
        st.markdown("### 📊 每日概览")
        summary_data = [
            {"指标": "总摄入", "数值": f"{data['营养摄入汇总']['总热量']} kcal", "详情": f"碳水: {data['营养摄入汇总']['总碳水']}g, 蛋白质: {data['营养摄入汇总']['总蛋白质']}g, 脂肪: {data['营养摄入汇总']['总脂肪']}g"},
        ...
        # --- 分类详情 Tabs ---
        t1, t2, t3, t4, t5, t6 = st.tabs(["🍽️ 饮食", "🏋️ 力量", "🏃 有氧", "💤 休息", "❤️ 活动", "📝 建议"])
        ...
        with t2:
            ...
            st.dataframe(pd.DataFrame(wo_meta), use_container_width=True, hide_index=True)
        ...
        with t3:
            ...
            st.dataframe(pd.DataFrame(cardio_table), use_container_width=True, hide_index=True)
        ...
        with t4:
            ...
            st.dataframe(pd.DataFrame(health_table), use_container_width=True, hide_index=True)
        ...
        with t5:
            ...
            st.dataframe(pd.DataFrame(body_table), use_container_width=True, hide_index=True)
